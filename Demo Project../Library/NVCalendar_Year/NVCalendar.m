...
-(NVCalendar *)createCalOfDay:(int)currentDay Month:(int)currentMonth Year:(int)currentYear MonthName:(NSString *)name
{
    ...
    currentMonthIs = currentMonth;
    currentYearIs=currentYear;
    ...
    lblMonthName.font = [UIFont fontWithName:@"HelveticaNeue" size:11.0];
...
-(void)lblDateTapped:(UITapGestureRecognizer *)tap//called when any date will be tapped
{
    ...
    NSString *strFormatted = lbl.text;
//    UILabel *lblMonth_Year = (UILabel *)[self viewWithTag:1999];
    NSString *msg = [NSString stringWithFormat:@"%@-%@-%@",strFormatted,[NSString stringWithFormat:@"%d",currentMonthIs],[NSString stringWithFormat:@"%d",currentYearIs]];
    NSString *stringToPass;
    
    if(currentMonthIs<=9)
    stringToPass = [NSString stringWithFormat:@"%d00%d",currentYearIs,currentMonthIs];
    else
        stringToPass = [NSString stringWithFormat:@"%d0%d",currentYearIs,currentMonthIs];
    [[NSUserDefaults standardUserDefaults] setValue:msg forKey:@"MonthTappedDetail"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    // All instances of TestClass will be notified
    
    NSDictionary *dictionary = [NSDictionary dictionaryWithObjectsAndKeys:stringToPass,@"yearAndMonth", nil];
                             
       [[NSNotificationCenter defaultCenter] postNotificationName:@"MonthTappedDetailClicked" object:dictionary];
//    [
//     postNotificationName:@"MonthTappedDetailClicked"
//     object:self];
